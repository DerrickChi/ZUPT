...
hold off;
mu = zeros(length(zv_window (:,1)),3);
center = zeros(length(zv_window (:,1)),1);
for i = 1:length(zv_window (:,1))
    mu(i,:) = mean(v(zv_window (i,1):zv_window (i,2),:),1);
    center(i) = round(mean([zv_window(i,1) zv_window(i,2)]));
end
v1 = zeros(size(v));
j = 1;
for i = 1:length(v(:,1))
    if i >= center(1) && i <= center(end)
        if (i > center(j+1))
            j = j + 1;
        end
        v1(i,:) = v(i,:) - (mu(j+1,:)*(i-center(j))+mu(j,:)*(center(j+1)-i))/(center(j+1)-center(j));
%     else
%         v1(i,:) = v(i,:);
    end
end
